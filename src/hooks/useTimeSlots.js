 ...
 * - Критическая бизнес-логика салона
 * - Используется в 3+ компонентах (TimeSlotPicker, AdminEdit, Rebook)
 * - ПОЧЕМУ: без проверки пересечений процедуры могут наслоиться по времени
...
export function useTimeSlots({
  ...
  const result = useMemo(() => {
    ...
    // ПОЧЕМУ: защита от silent fail при неполных входных данных
