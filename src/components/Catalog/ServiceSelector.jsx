...
const SORT_OPTIONS = [
  { 
    field: 'popular', 
    labelKey: 'catalog.sort.popular', 
    shortLabel: 'Популярность',
  ...
  { 
    field: 'name', 
    labelKey: 'catalog.sort.name', 
    shortLabel: 'А-Я',
    defaultDirection: 'asc' 
  },
  { 
    field: 'category', 
    labelKey: 'catalog.sort.category', 
    shortLabel: 'Категория',
    defaultDirection: 'asc' 
  },
];
...
export default function ServiceSelector({
  ...
  const handleSearchChange = (e) => {
    const { value, errorKey } = processSearchInput(e.target.value);
    setSearchQuery(value);
    setSearchError(errorKey);
  };
    ...
    <div className="service-selector">
      ...
      <div className="service-selector__filters">
        <Input
          ...
          maxLength={SEARCH_INPUT_MAX}
          error={searchError ? t(searchError) : null}
          helperText={
            searchQuery
              ? t('catalog.search.charCount', {
                  current: searchQuery.length,
                  max: SEARCH_INPUT_MAX,
                })
              : null
          }
